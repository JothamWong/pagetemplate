...
def create_template(width, height, title, margin, line_spacing, line_color, background_color):
    image = Image.new('RGB', (width, height), background_color)
    draw = ImageDraw.Draw(image)
    # Left margin
    draw.line([(margin, 0), (margin, height)], fill=line_color)
    # Horizontal lines
    for y in range(title, height, line_spacing):
...
if __name__ == "__main__":
    ...
    args.add_argument("--title", type=int, default=75, help="Height of page")
